...
interface AreaSwitchProps {
  ...
  onToggle?: () => void;
  loading?: boolean;
  /** Operators: display status only (no breaker control). */
  readOnly?: boolean;
}

export function AreaSwitch({ name, state, className, onToggle, loading, readOnly }: AreaSwitchProps) {
  const isOn = state === 'ON';
  ...
  return (
    <button
      ...
      className={cn(
        ...
        !locked && 'cursor-pointer hover:brightness-110 active:scale-[0.98]',
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <div className="text-xs text-muted-foreground">
            {readOnly ? 'View only (operator)' : 'Distribution Zone'}
          </div>
        </div>
      </div>
